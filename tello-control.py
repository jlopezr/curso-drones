...
class Interface:
    ...
    def run(self):
        #todo esto es necesario no eres un drone ya?
        self.drone = Drone()
        ...
        threading.Thread(target=self.interface_thread).start()
    ...
    def interface_thread(self):
        ...
        while not self.terminate:
            ...
            if steps % 15 == 1:
                ...
                pad = self.drone.tello.get_mission_pad_id()
                if pad>0:
                    print("PAD", pad,":",
                          self.drone.tello.get_mission_pad_distance_x(),
                          self.drone.tello.get_mission_pad_distance_y(),
                          self.drone.tello.get_mission_pad_distance_z())
                ...
                self.hud_elements = [
                    ...
                    (f"Distance: {self.drone.tello.get_distance_tof():.2f}cm"), #TODO si da 10 es que no lee nada, a partir de 30cm
                    (f""),
                    #todo en que unidad? z = 1000
                    ...
                    (f""),
                    (f"Temp:     {self.drone.tello.get_temperature():.1f}"),
                    
                ]

            f = self.grab_frame()
            if f is not None:
                # TODO Hacer video activable
...
##                self.video.write(video_f)
            ...
            #TODO this is needed? #creo que iria mejor un clock de pygame
            #sleep(1 / 30)
    ...
    def capture_controls(self):
        ...
        while True:
            ...
            if get_key("m"):
                self.drone.tello.enable_mission_pads()

            if get_key("t"):
                self.drone.tello.enable_mission_pads()
                self.drone.takeoff()
            elif get_key("l"):
                self.drone.tello.disable_mission_pads()
                self.drone.land()

##TODO no funciona: AttributeError: 'Interface' object has no attribute 'latest_image'
